...
export default class Me extends React.Component {
  ...
  list (items) {
    if (items.length > 0) {
      return items.map((v, i) => <Item key={i} multipleLine extra={<div dangerouslySetInnerHTML={{__html: v.text.replace(/size="\d"/, '')}}/>} arrow="horizontal" onClick={this.onClick.bind(this, v)}>{v.title}<Brief>副标题</Brief></Item>)
    } else {
      return <div class="order-nothing">你还没有借款记录哦~</div>
    }
  }
  render () {
