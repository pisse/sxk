...
import { Icon, List, Button, Checkbox, Popup, Modal } from 'antd-mobile'
  ...
  fetchData (params) {
    ...
    post('http://credit.xianjincard.com/credit-loan/get-confirm-loan', params).then((result) => {
      ...
      if (result.data.code === -1) {
        Toast.info(result.data.message, 0.8, () => {
          redirect.goBack()
        })
        return
      }
    ...
    return (
      <div class="loan-confirm">
        ...
        <AgreeItem data-seed="logId" defaultChecked={data.agree} onChange={this.onAgree.bind(this)}>
          我已阅读并同意<a href={data.protocol_url}>《借款协议》</a><a href="">《平台服务协议》</a><br/><a href="">《授权扣款委托书》</a>
        </AgreeItem>
        <img src="/assets/img/mobile/misc/bottom-logo.png"/>
      </div>
    )
  }
}
