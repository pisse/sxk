...
const config = {
    ...
    output: {
        path: paths.dist(),
        // publicPath: DEBUG ? `http://${HOST}:${PORT}/` : `http://192.168.40.210/h5/pc/dist/`,
        publicPath: DEBUG ? `http://${HOST}:${PORT}/` : '//www.xianjincard.com/',
        filename: `js/[name].js?[${HASHTYPE}]`, //`js/[name].[${HASHTYPE}].js`,
    ...
    plugins: [
        ...
        ...DEBUG ? [
            ...
            new ExtractTextPlugin(`assets/css/[name].css?[${HASHTYPE}]`)
            // 移除未引用的样式
            // new PurifyPlugin({}),
        ]
    ],
...
// multi page
{
    let files = glob.sync(paths.src(`views/{,**/}*.{htm,html,pug}`), {
        ...
        files.map(file => {
            ...
            (options.filename = options.filename.replace(/src\/views/, 'dist'));
